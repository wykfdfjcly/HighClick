...
using HighClick.Migrations.SeedData;
...
namespace HighClick.Tests
{
    public abstract class HighClickTestBase : AbpIntegratedTestBase<HighClickTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected HighClickTestBase()
        {
        ...
        protected void UsingDbContext(Action<HighClickDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<HighClickDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HighClickDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HighClickDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HighClickDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HighClickDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<HighClickDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HighClickDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HighClickDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HighClickDbContext, Task<T>> func)
        {
            T result;
